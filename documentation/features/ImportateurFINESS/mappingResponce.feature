...
Contexte: Le FinessBatchImpoter doit importer des données à partir d'un fichier texte et les envoyer au FINESS+ via son API. Le système externe qui demande l'importation peut avoir besoin de recevoir une réponse de traitement détaillée. Les détails de la réponse peuvent être configurés.
...
Scénario: Utilisation de la configuration de la réponse pour l'importation de données
    ...
    Et le FinessBatchImpoter génère une réponse de traitement en utilisant la configuration enregistrée
    Et le FinessBatchImpoter envoie la réponse de traitement au système externe qui demande l'importation en batch
